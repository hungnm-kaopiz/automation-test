 ...
 *   - Article CMS pages: copy live title/description/h1/breadcrumb from APP_BASE_URL
...
import { assertResolvedTitleParenStyle, normalizeItem } from "./normalize-seo.ts";
 ...
 * Returns the canonical pathname and updated fullpath (always http://localhost:3000).
 */
...
async function run(): Promise<void> {
  console.info(`[resolve-output] API_BASE_URL=${API_BASE}`);
  ...
  if (master.prefecturesById.size === 0) {
    throw new Error(
      `No prefecture data returned from ${API_BASE}. Set API_BASE_URL and ensure the API is running.`,
    );
  }
