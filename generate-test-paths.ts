...
async function fetchAllIds(): Promise<{
  ...
  if (prefectures.length === 0) {
    throw new Error(
      `No prefectures returned from ${API_BASE}. Set API_BASE_URL and ensure the API is running.`,
    );
  }
  ...
  if (failures.length > 0) {
    ...
    throw new Error(
      `${failures.length} generated listing URL(s) have no facility data. ` +
        "Ensure API_BASE_URL points to an environment with facility records.",
    );
  }
...
async function run(): Promise<void> {
  console.info(`[generate-paths] API_BASE_URL=${API_BASE}`);
