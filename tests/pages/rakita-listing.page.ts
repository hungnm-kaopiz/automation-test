...
import { type ArticleDetailContext } from "../support/article-cms.js";
...
export class RakitaListingPage extends BasePage {
  ...
  async readArticleDetailContext(): Promise<ArticleDetailContext | null> {
    return this.page.evaluate(() => {
      const headline = document.querySelector("h1")?.textContent?.trim() ?? "";
      if (!headline) return null;

      const readUserCard = (section: HTMLElement) => {
        const info = section.querySelector(":scope > div:last-child");
        const name =
          info?.querySelector(".font-bold")?.textContent?.trim() ?? "";
        const bio =
          info
            ?.querySelector(".whitespace-break-spaces")
            ?.textContent?.trim() ?? undefined;
        const websiteAnchor = info?.querySelector('a[href][rel~="noopener"]');
        const websiteUrl = websiteAnchor?.getAttribute("href") ?? undefined;
        const imageUrl = section.querySelector("img")?.getAttribute("src") ?? undefined;

        return { name, bio, websiteUrl, imageUrl };
      };

      const authorSection = document.getElementById("author-section");
      const supervisorSection = document.getElementById("supervisor");

      const authorCard = authorSection ? readUserCard(authorSection) : undefined;
      const supervisorCard = supervisorSection
        ? readUserCard(supervisorSection)
        : undefined;
      ...
      return {
        ...
        ...(authorCard?.name
          ? {
              author: {
                ...
                ...(authorCard.websiteUrl
                  ? { websiteUrl: authorCard.websiteUrl }
                  : {}),
              },
            }
          : {}),
        ...(supervisorCard?.name
          ? {
              supervisor: {
                name: supervisorCard.name,
                ...(supervisorCard.bio ? { bio: supervisorCard.bio } : {}),
                ...(supervisorCard.imageUrl
                  ? { imageUrl: supervisorCard.imageUrl }
                  : {}),
                ...(supervisorCard.websiteUrl
                  ? { websiteUrl: supervisorCard.websiteUrl }
                  : {}),
              },
            }
          : {}),
  ...
  async readSeoTechnicalMetadata(): Promise<SeoTechnicalMetadata> {
    return this.page.evaluate(() => {
      ...
      return {
        ...
        ogSiteName: propertyContent("og:site_name"),
        ogLocale: propertyContent("og:locale"),
